...
if (system.args.length < 3) {
    ...
    dest = system.args[2];
    url = 'http://maps.googleapis.com/maps/api/directions/xml?origin=' + origin + '&destination=' + dest + '&units=imperial&mode=driving&sensor=false';
    en_url = encodeURI(url);
    page.open(en_url, function (status) {
        if (status !== 'success') {
