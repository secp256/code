...
curl "https://accounts.google.com/accountLoginInfoXhr" \
    ...
    --data "Email=${username}&requestlocation=${requestlocation}&bgresponse=js_disabled&Page=PasswordSeparationSignIn&GALX=${GALX}&gxf=${gxf}&hl=en&_utf8=%E2%98%83&pstMsg=1&checkConnection=youtube%3A1127%3A1&checkedDomains=youtube&rmShown=1" \
...
curl "https://accounts.google.com/ServiceLoginAuth" \
    ...
    -H "User-Agent: ${user_agent}" \
    -H "${content_type_header}" \
    --data "Page=PasswordSeparationSignIn&GALX=${GALX}&gxf=${gxf}&hl=en&_utf8=%E2%98%83&pstMsg=1&dnConn=&checkConnection=youtube%3A1127%3A1&checkedDomains=youtube&identifiertoken=&identifiertoken_audio=&identifier-captcha-input=&Email=${username}&Passwd=${password}&PersistentCookie=yes&ProfileInformation=${ProfileInformation}&rmShown=1&bgresponse=js_disabled" \
